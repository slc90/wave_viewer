...
pub fn show_channel_and_wave(ctx: &Context, ui: &mut Ui, channel_and_waves: &ChannelAndWaveState) {
    ...
    // 有数据时覆盖上面的值
    if channel_and_waves.all_waves.len() != 0 {
        data_length = channel_and_waves.all_waves[0].data.len();
    ...
    Plot::new("channel_and_wave")
        ...
        // 最小值比0小一些,左边要用来画通道名
        .default_x_bounds(-0.02, x_max + 0.00001)
        .show_grid(false)
        .x_axis_label("Time(s)")
        // y轴不要显示刻度
        ...
        .show(ui, |plot_ui| {
            channel_and_waves.all_waves.iter().for_each(|single_wave| {
                // 画通道名
                plot_ui.text(Text::new(
                    &single_wave.channel_name,
                    PlotPoint::new(-0.01, single_wave.offset),
